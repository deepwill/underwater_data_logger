...
struct File_t file;			// important data/variables for working with files
...
//*******************************************************************************************************************************
// function prototypes that are only used in this file!
#if (MMC_LS==TRUE)
...
// ffopen would create the file if it does not exist!
// *******************************************************************************************************************************
...
//*******************************************************************************************************************************
// Two ways to open a file, file exists:(return MMC_FILE_OPENED) or has to be created: (return MMC_FILE_CREATED)
// firstly it is checked if file exists in directory. Secondly it is decided if file will be opened or created.
// -when opening - known clusters are looked for to a maximum of: MAX_CLUSTERS_IN_ROW in a row. Following the 1. sector will be loaded to buffer fat.sector, and read by ffread...
// -when creating free clusters are looked for, maximum: MAX_CLUSTERS_IN_ROW in row. Then struct file filled up.
// now the file file is filled on sd card. Here also is being checked if sufficient space is available in current directory.
// possibly the first sector is loaded again. Now first free sector is known and it can be written to it.
//*******************************************************************************************************************************
uint8_t ffopen( uint8_t name[], uint8_t rw_flag){

	uint8_t file_flag = fat_loadFileDataFromDir(name);	//check if file present. If present, load to file struct.

	if( file_flag==TRUE && rw_flag == 'r' ){					/** if file exists prepare everything to read **/
		fat_getFatChainClustersInRow( file.firstCluster );		// look for chained clusters from fat-chain.
		file.name = name;
		#if (MMC_MULTI_BLOCK==TRUE && MMC_OVER_WRITE==FALSE)
		...
		#else
			fat_loadSector( chain.startSectors );				//load first 512 bytes to buffer:sector.
		#endif
		return MMC_FILE_OPENED;
	}

	#if (MMC_WRITE==TRUE)										//creating is writing !
	if( file_flag==FALSE && rw_flag == 'c' ){					/** file does not exist, create !	(only if write flag is set to TRUE)**/
		file.name = name;										//"save" pointer
		fat_getFreeClustersInRow( 2 ); 							//look for free clusters from fat. Actually, this is only needed in fat_getFreeRowsOfDir and later in ffwrite !
		fat_makeFileEntry(name,0x20);							//CREATE FILE ON SD CARD
		#if (MMC_MULTI_BLOCK==TRUE && MMC_OVER_WRITE==FALSE)
			multiblock_flag = FALSE;
			mmc_multi_block_start_write(fat.currentSectorNr);
		#endif
		fat.bufferDirty = TRUE;									// buffer dirty because written and still not on sd card.
...
//*******************************************************************************************************************************
// closes file operation. only necessary when written/overwritten. 
//There are 2 possibilities:
// 1. file is being closed and it was written more the old file length.
// 2. file is being closed and the changes are within the file size - only current sector has to be written.
// case one is complicated, it has to be determined how many sectors have to be filled in order to chain these together
// then the new file length has to be determined. Finally either (case 2) only current sector is written or
// the updated file entry and the cluster (these are chained together, see also fileUpdate()).
//*******************************************************************************************************************************
uint8_t ffclose(){
